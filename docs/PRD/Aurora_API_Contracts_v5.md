...
## 67. Project Management Endpoints (PRD §3A)

### POST /api/projects
Creates a new project. Requires `project-lead` role or above.
**Request body:** `{ projectName, startDate, customerName, modulesSelected, therapeuticArea, endDate?, projectDescription?, linkedStudyRef?, countriesOfSubmission? }`
**Response 201:** `Project`
**Validation:** `startDate` cannot be a past date; `projectName` must be unique within client account; `therapeuticArea` mandatory.

### GET /api/projects
Returns all projects visible to the authenticated user.
**Response 200:** `Project[]`
**Query params:** `status`, `ta`, `module`, `customer`, `dateFrom`, `dateTo`

### GET /api/projects/:projectId
**Response 200:** `Project` with full document summary.

### PATCH /api/projects/:projectId/status
Updates project status.
**Request body:** `{ status: ProjectStatus; reason?: string }`
**Response 200:** Updated `Project`
**Rules:** `Closed → [any]` returns 403 — terminal state. Reason is mandatory for `On Hold`.

### GET /api/projects/:projectId/dashboard
Returns project dashboard metrics.
**Response 200:** `{ costSummary, timeSummary, performanceSummary, documents: DocumentSummary[] }`

---

## 68. Voice Note Endpoints (PRD §19.2)

### POST /api/ideation-artefacts/:artefactId/voice-notes
Uploads a voice note recording.
**Request body:** `multipart/form-data` — audio file + `{ contributorId, contributorName }`
**Response 201:** `VoiceNoteRecord` with `transcriptText` (auto-transcribed)
**Response 409:** `{ error: 'MAX_CONTRIBUTORS_EXCEEDED', message: 'Maximum 3 voice note contributors per project per ideation cycle.' }` — hard limit from PRD §19.2.

### GET /api/ideation-artefacts/:artefactId/voice-notes
**Response 200:** `VoiceNoteRecord[]`

---

## 69. Notification Endpoints (PRD §19.5)

### GET /api/notifications
Returns notifications for the authenticated user.
**Response 200:** `NotificationRecord[]`
**Query params:** `unread`, `eventType`, `projectId`

### POST /api/notifications/:id/read
Marks a notification as read.
**Response 200:** Updated `NotificationRecord`

### POST /api/ideation-projects/:projectId/kol-invite
Sends KOL review invitation. Creates a secure one-time token.
**Request body:** `{ kolEmail, kolName, kolMobile?, cardIds: string[] }`
**Response 200:** `{ token: string; expiresAt: string }` — token expires in 7 days.
**Side effects:** Email sent to KOL with secure review link at `/kol-review/:token`.

### POST /api/ideation-projects/:projectId/kol-remind
Manually triggers a KOL reminder. Escalation is automatic per FR-E-013.
**Request body:** `{ reminderNumber: 1 | 2 | 'escalation' }`
**Response 200:** `{ sent: true; channel: string[] }`

---

## 70. Localisation Endpoints (sE07)

### GET /api/ideation-cards/:cardId/locales
Returns localisation records for a content card.
**Response 200:** `LocaleRecord[]`

### POST /api/ideation-cards/:cardId/locales
Creates a new locale record and sends affiliate MA review link.
**Request body:** `{ localeCode, country, affiliateContactEmail, detail? }`
**Response 201:** `LocaleRecord`
**Side effects:** Email sent to affiliate contact with localised PDF and review link.

### PATCH /api/ideation-cards/:cardId/locales/:localeId
Updates locale record status.
**Request body:** `{ status: LocaleRecord['status'] }`
**Response 200:** Updated `LocaleRecord`

---

## 71. FR-E-013 — KOL Escalation Endpoint

### GET /api/ideation-projects/:projectId/kol-escalation-status
Returns KOL review escalation state for a project.
**Response 200:** `{ escalated: boolean; escalatedAt?: string; reminder1SentAt?: string; reminder2SentAt?: string; escalationNote?: string }`

The escalation is triggered automatically by the platform scheduler:
- Day 3: Reminder 1 sent (FR-E-013)
- Day 5: Reminder 2 sent (FR-E-013)
- Day 7: Escalation email to Ideation Lead + MA Team Lead (FR-E-013)

All escalation events are written to the audit trail.

---

## 72. FR-D-002 — eCTD Version Selection

### GET /api/ectd-versions
Returns available eCTD versions configured by Admin.
**Response 200:** `[{ version: 'v3.2.2'; label: 'eCTD v3.2.2 (current)' }, { version: 'v4.0'; label: 'eCTD v4.0 (emerging)' }]`

### PATCH /api/submissions/:submissionId/ectd-version
Updates eCTD version for a submission. Locked after Stage 3 entry.
**Request body:** `{ version: 'v3.2.2' | 'v4.0' }`
**Response 200:** Updated submission.
**Response 409:** `{ error: 'VERSION_LOCKED', message: 'eCTD version cannot be changed after Stage 3.' }`

---

## 73. FR-D-023 — Safety Signal Cross-Module Trigger

### POST /api/regulatory-intelligence/safety-signal
Records a safety signal and auto-creates label update tasks.
**Request body:** `{ signalType: string; description: string; affectedSections: string[]; assignedTo: UserId }`
**Response 201:** `{ signalId: string; tasksCreated: { section: string; module: ModuleKey }[] }`
**Side effects:** Label update task created in SmPC §4.4 and USPI Warnings & Precautions; trigger logged to sD11 cross-functional triggers panel.

---

## 74. FR-D-025 — Regulatory Framework Alert Cross-Module Push

### POST /api/regulatory-intelligence/alerts/:alertId/push
Pushes a regulatory framework alert to all applicable modules.
**Request body:** `{ targetModules: ModuleKey[] }`
**Response 200:** `{ pushed: ModuleKey[]; alertRecord: RegulatoryAlert }`
**Side effects:** Alert card appears in cross-module alert panels for each target module. Audit entry written.

---

## 75. Payment Gateway Endpoints (PRD §8.3)

### GET /api/billing/subscription
Returns current subscription plan and usage.
**Response 200:** `{ plan: string; usageThisMonth: UsageRecord; nextInvoiceDate: string; paymentGateway: string }`

### POST /api/billing/checkout
Initiates a payment gateway session (gateway selected at Admin level).
**Request body:** `{ planId: string; returnUrl: string }`
**Response 200:** `{ checkoutUrl: string }` — redirect URL to gateway (Stripe or configured alternative).
...
**Request body:** `{ gatewayToken: string }` — tokenised payment method from gateway SDK.
**Response 200:** `{ updated: true }`

---

## 76. Master Library — Partial Reuse & Best Practices (PRD §7.6)

### GET /api/master-library/sections
Returns tagged sections available for partial reuse.
**Query params:** `module`, `projectId`, `ta`, `docType`, `q` (full-text search)
**Response 200:** `TaggedSection[]`

### POST /api/master-library/sections/:sectionId/reuse
Pulls a tagged section into a new document.
**Request body:** `{ targetDocumentId: string; position?: number }`
**Response 201:** `{ inserted: true; provenanceChain: string[] }` — provenance chain preserved from source to new document.

### GET /api/master-library/best-practices
Returns Best Practices items (Super Admin maintained).
**Response 200:** `BestPractice[]`
**Query params:** `module`, `category`

---

## 77. Terms & Conditions Endpoints (PRD §20)

### GET /api/terms
Returns current T&C version.
**Response 200:** `{ version: string; effectiveDate: string; content: string; disclaimer: string }`

### POST /api/terms/accept
Records user T&C acceptance.
**Request body:** `{ version: string }` — must match current version.
**Response 200:** `{ accepted: true; timestamp: string }`
**Side effects:** Audit entry: `{ userId, action: 'TC_ACCEPTED', tcVersion, timestamp }`.

---

## 78. Regulatory Framework Registry Endpoints (PRD §11A.4)

### GET /api/regulatory-frameworks
Returns all tracked frameworks. Filterable by module.
**Query params:** `module`, `issuer`, `status`
**Response 200:** `RegulatoryFramework[]`

### PATCH /api/regulatory-frameworks/:frameworkId
Updates a framework entry (Super Admin only).
**Request body:** `{ status?, changeDate?, changeSummary?, version? }`
**Response 200:** Updated `RegulatoryFramework`

---

## 79. Social Listening Extended Endpoints

### GET /api/social-listening/alerts
Returns all social listening alerts.
**Query params:** `projectId`, `channel`, `resolved`, `dateFrom`, `dateTo`
**Response 200:** `SocialListeningAlert[]`

### POST /api/social-listening/alerts/:alertId/review
Records MA Lead review of a flagged comment.
**Request body:** `{ verdict: 'no action required' | 'content modified' | 'auto-stop triggered'; note: string }`
**Response 200:** Updated `SocialListeningAlert`
**Side effects:** Audit entry; notification sent to Content Calendar Manager if auto-stop triggered.

---

## 80. Project Dashboard Report Export

### GET /api/projects/:projectId/report
Generates a project-level report.
**Query params:** `format: 'pdf' | 'csv'`
**Response 200:** Binary file stream (PDF or CSV).
**Access control:** Project Lead role or above only.

---

## 81. All Projects Tab Endpoint

### GET /api/projects/all
Returns all projects for the client account (all statuses).
**Response 200:** `Project[]` with summary fields only: `{ projectId, projectName, customerName, modulesSelected, therapeuticArea, startDate, status, projectLeadName }`.
**Access control:** All authenticated users.

---

## 82. DD-C-003 / DD-C-004 / DD-C-005 — Content Design Decisions (API Impact)

**DD-C-003 — Source read-only:** `GET /api/medical-writing/artefacts/:id` always returns `{ editable: false }` for source documents. Any `PATCH` on a source document returns 403 with `{ error: 'SOURCE_READ_ONLY' }`.

**DD-C-004 — PAO annotation label:** The API field is `patientAdvocateAnnotations` (not `paoLayer`). Endpoint: `GET/POST /api/medical-writing/documents/:id/patient-advocate-annotations`.

**DD-C-005 — FK readability gate:** `POST /api/medical-writing/documents/:id/mlr-submit` returns 422 `{ error: 'FK_GRADE_EXCEEDED', grade: number, threshold: 8 }` when content type is `'PIL'` or `'EU_CTR_PLS'` and FK grade > 8.
