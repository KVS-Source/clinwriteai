...
## Module D — Missing Acceptance Criteria (AC-D-028 through AC-D-051)

*(sD01, sD11, sD12 — home screen, regulatory intelligence, final output portfolio)*

---

### § AC-D-028 — sD01: Dashboard pipeline chips show correct stage counts

**Screen:** sD01 — Regulatory Writing Home
**FR:** FR-D-001

**Given** the Regulatory Writing Home screen loads
**When** the user views the pipeline summary strip
**Then** four stage chips are shown: Stage 1 (Source Data), Stage 2 (Module 2 Authoring), Stage 3 (Modules 1 & 3–5), Stage 4 (Integrated Review), Stage 5 (Publishing), Stage 6 (Submission) — with counts matching active submissions per stage. At minimum, the demo submission `sub-001` appears in Stage 2 with the `NDA/MAA` label.

---

### § AC-D-029 — sD01: RI alerts panel shows unread count

**Screen:** sD01 — Regulatory Writing Home
**FR:** FR-D-024

**Given** the Regulatory Writing Home screen loads
**When** unread regulatory intelligence alerts exist
**Then** the RI panel shows the unread count as an amber badge and at least one alert card with framework name, issuer, and effective date.

---

### § AC-D-030 — sD01: ECTd publishing mini-panel shows compilation progress

**Screen:** sD01 — Regulatory Writing Home
**FR:** FR-D-019

**Given** Stage 5 publishing is active for a submission
**When** the Regulatory Writing Home is viewed
**Then** the eCTD publishing mini-panel shows: X of Y sections compiled, a percentage, and the continuous publishing indicator.

---

### § AC-D-031 — sD11: Framework monitor shows all tracked frameworks

**Screen:** sD11 — Regulatory Intelligence
**FR:** FR-D-024, DD-D-005

**Given** the Regulatory Intelligence screen loads
**When** the framework monitor panel is viewed
**Then** at least 10 regulatory frameworks are listed. The table shows: framework name, issuer, last updated date, and current status chip. Status chips: `✓ Current` (green) or `⚠ Updated` / `⚠ Draft revision` (amber). A "Show all 17 →" toggle reveals the full list.

---

### § AC-D-032 — sD11: Unread alert cards show full detail

**Screen:** sD11 — Regulatory Intelligence
**FR:** FR-D-024, DD-D-005

**Given** unread regulatory framework alerts exist
**When** the user views the alert panel in sD11
**Then** each alert card shows: framework name, issuer, effective date, kind chip (Guidance revision / Regulation update), full change summary text, affected sections chip, and modules the alert was pushed to. "Acknowledge" button is active on unread alerts.

---

### § AC-D-033 — sD11: Acknowledging an alert writes to audit trail

**Screen:** sD11 — Regulatory Intelligence
**FR:** FR-D-024

**Given** a regulatory alert is unread
**When** the user clicks "Acknowledge"
**Then** the alert transitions to acknowledged state, the acknowledgement is written to the audit trail (user name + timestamp), and the action is immutable. The alert cannot be re-set to unread.

---

### § AC-D-034 — sD11: Cross-functional triggers panel

**Screen:** sD11 — Regulatory Intelligence
**FR:** FR-D-025 (cross-module trigger)

**Given** a Module A CSR has been updated or a safety signal has been flagged
**When** the cross-functional triggers panel is viewed
**Then** the trigger card shows: event name, date, affected sections, and current state (e.g., ⚠ Review required, ✓ Acknowledged, ● In progress). At least 3 trigger cards are shown in the demo.

---

### § AC-D-035 — sD11: Safety signal auto-creates label update task (FR-D-023)

**Screen:** sD11 — Regulatory Intelligence
**FR:** FR-D-023

**Given** a safety signal trigger card is visible in sD11
**When** the user views the trigger detail
**Then** the trigger shows: "Safety signal flagged — label update task auto-created: SmPC §4.4 Special Warnings." and "USPI Warnings and Precautions" with assigned user and state "● In progress."

---

### § AC-D-036 — sD11: Selected framework shows affected sections

**Screen:** sD11 — Regulatory Intelligence
**FR:** FR-D-024

**Given** the user selects a framework alert card
**When** the right panel updates to show the selected framework
**Then** the affected sections panel lists: CTD section code, last review date, regulatory basis, and a note (e.g., "⚠ Guideline updated since last review").

---

### § AC-D-037 — sD12: Final output banner shows ACK2 and PDUFA date

**Screen:** sD12 — Final Output & Portfolio
**FR:** FR-D-021

**Given** a submission has received ACK2 confirmation
**When** the Final Output Portfolio is viewed
**Then** a prominent banner states: "ACK2 received — format validation passed" with the FDA ESG transmission date, estimated review period, and PDUFA action date.

---

### § AC-D-038 — sD12: Compliance provenance chain is fully visible

**Screen:** sD12 — Final Output & Portfolio
**FR:** FR-D-019, DD-D-003

**Given** the compliance provenance chain section is displayed
**When** the user views the Regulatory Writing provenance entries
**Then** all 6 stage entries are visible without any click or expand interaction: Stage 1 through Stage 6, each with date, responsible person, and action. No entries are collapsed.

---

### § AC-D-039 — sD12: Master Library cards show correct names

**Screen:** sD12 — Final Output & Portfolio
**FR:** FR-D-027

**Given** cards have been pushed to the Master Library from a completed submission
**When** the Master Library section is viewed
**Then** all 5 card names are shown: CTD 2.5 Clinical Overview, CTD 2.7 Clinical Summary, SmPC v1.0, RMP Core Document, HA Response Template Bundle — FDA Day 120.

---

### § AC-D-040 — sD12: Portfolio kanban shows correct lane counts

**Screen:** sD12 — Final Output & Portfolio
**FR:** FR-D-019

**Given** the Submission Portfolio tab is selected
**When** the kanban is viewed
**Then** 4 lanes are shown: Authoring (Stage 1–3), Super Review (Stage 4), Publishing (Stage 5), Submitted (Stage 6). The Submitted lane shows at least 2 cards: Veloricept NDA and AURELIA-101 IND. The Super Review lane shows at least 1 card with "3 of 6 roles signed."

---

### § AC-D-041 — sD12: ODD assessment shows 3 rows with verdicts

**Screen:** sD12 — Final Output & Portfolio
**FR:** FR-D-026

**Given** an ODD assessment is active for AURELIA-101
**When** the ODD section is viewed
**Then** 3 assessment rows are shown: EU prevalence (3.2 per 10,000 — threshold ≤5 → ✓ Meets threshold), US patient estimate (~178,000 — threshold <200,000 → ✓ Meets threshold), Medical plausibility (pending Clinical Lead sign-off → ○ Pending). Score shown: 84% · Likely eligible for designation.

---

### § AC-D-042 — sD12: HA Correspondence panel shows 3 rows

**Screen:** sD12 — Final Output & Portfolio
**FR:** FR-D-022

**Given** the HA Correspondence panel is shown
**When** the user views it
**Then** 3 rows appear: Veloricept NDA · FDA (ACK2 received), Veloricept NDA · EMA (Not yet transmitted), AURELIA-101 IND · EMA (ACK3 received). Status chips use green for received, grey for pending.

---

### § AC-D-043 — sD12: Timeline milestones show PDUFA date prominently

**Screen:** sD12 — Final Output & Portfolio
**FR:** FR-D-021

**Given** the predicted timeline strip is shown
**When** the user views the submission timeline
**Then** 5 milestones appear: Transmitted ✓, ACK1 ✓ (6 min), ACK2 ✓ (2h 25m), ACK3 est. (31 Oct), PDUFA (16 Aug 2027). The PDUFA milestone is visually prominent — bold, larger font weight.

---

### § AC-D-044 — Project Management: Closed project is read-only (PRD §3A)

**FR:** FR-D-001 (project layer rule)

**Given** a project has `status: 'Closed'`
**When** any user (including Super Admin) attempts to create, edit, or delete a document within the project
**Then** the API returns 403 with `{ error: 'PROJECT_CLOSED' }`. No write operation succeeds. The platform UI shows all documents in read-only mode.

---

### § AC-D-045 — Project Management: Initiated status on creation

**FR:** FR-D-001

**Given** a user with Project Lead role creates a new project
**When** the project is submitted
**Then** the project receives `status: 'Initiated'`, a UUID `projectId`, and the user is navigated to the Project Dashboard. All selected modules are unlocked.

---

### § AC-D-046 — Project Management: All Projects tab shows all statuses

**FR:** FR-D-001

**Given** the All Projects tab is selected on the main dashboard
**When** the list loads
**Then** all projects visible to the user are shown with: Project Name, Project ID, Customer Name, Modules (coloured dots), TA, Start Date, Status, Project Lead. Filter controls are present for Status, TA, Module, Customer, Date Range.

---

### § AC-D-047 — Terms & Conditions acceptance at sign-up (PRD §20)

**FR:** NFR

**Given** a new user signs up for the platform
**When** the registration flow reaches the T&C step
**Then** the full T&C text is displayed with the regulatory disclaimer. The "I have read and agree" checkbox must be checked before the Submit button is enabled. Acceptance is recorded in the audit trail with timestamp and T&C version.

---

### § AC-D-048 — Regulatory disclaimer in platform footer

**FR:** NFR (PRD §20.2)

**Given** any authenticated user is viewing any platform screen
**When** the footer is visible
**Then** the regulatory disclaimer appears: "This platform is a productivity and authoring tool. It is not a substitute for regulatory advice. All content generated must be reviewed by qualified regulatory professionals before submission to any health authority."

---

### § AC-D-049 — FR-D-002: eCTD version is selectable at Stage 1

**Screen:** sD02 — Submission Setup & Strategy
**FR:** FR-D-002

**Given** the user is setting up a new submission
**When** the eCTD version selector is shown
**Then** two options are available: eCTD v3.2.2 (pre-selected as default) and eCTD v4.0. The selected version drives the eCTD Granularity Map validation in sD03. Once the submission enters Stage 3, the version is locked and cannot be changed.

---

### § AC-D-050 — FR-D-002: eCTD v4.0 selection updates map rules

**Screen:** sD03 — eCTD Granularity Map
**FR:** FR-D-002

**Given** the user selected eCTD v4.0 at Stage 1
**When** the eCTD Granularity Map is generated
**Then** the map applies v4.0 validation rules (different from v3.2.2). A banner note states: "eCTD v4.0 selected — PMDA mandatory; FDA/EMA transition pathway."

---

### § AC-D-051 — Regulatory Framework Registry accessible to all users

**FR:** FR-D-024

**Given** any authenticated user navigates to the Regulatory Intelligence panel
**When** the framework monitor table is viewed
**Then** all tracked frameworks are listed in read-only mode. The Super Admin can edit framework entries via Admin settings; other users can view only.

---

## Module E — Missing Acceptance Criteria (AC-E-024 through AC-E-039)

*(sE01, sE07, sE08, sE09, sE10 — home, calendar, publishing, final output, standards)*

---

### § AC-E-024 — sE01: Home screen shows all project states

**Screen:** sE01 — Ideation & Publishing Home
**FR:** FR-E-001

**Given** the Ideation & Publishing Home screen loads
**When** the user views the project card list
**Then** three card states are visible: (1) a Stage 4 Approved project with teal badge, MA approval stamp, and card counts; (2) a Stage 2 Under Review project with amber badge; (3) a Stage 1 Uploaded/Blocked project with rose `#BE123C` badge and source gate failure reason. All TA chips are visible on each card.

---

### § AC-E-025 — sE01: Blocked project shows source gate failure reason

**Screen:** sE01 — Ideation & Publishing Home
**FR:** FR-E-001, DD-E-002

**Given** a project has a source artefact with `approvalStatusCheck: 'blocked'`
**When** the project card is displayed on sE01
**Then** the card shows a rose-coloured "Blocked" badge and the failure reason: "Source gate failed — document 'In Authoring' in Module A." The "Proceed" action is absent.

---

### § AC-E-026 — sE07: Localisation panel is accessible from scheduling

**Screen:** sE07 — Content Calendar
**FR:** FR-E-015

**Given** an approved content card has been scheduled
**When** the user clicks "Localise" on a card in the queue panel
**Then** the Localisation panel opens showing: parent card name, approval date, locale rows (code, country, status, detail), and "+ Add locale" action. Every localised version links back to the parent card record.

---

### § AC-E-027 — sE07: Local MA review link is sent to affiliate contact

**Screen:** sE07 — Content Calendar
**FR:** FR-E-015

**Given** the user adds a new locale record
**When** the locale is saved
**Then** an email is sent to the affiliate contact email with the localised PDF and review link in the local language. The locale record transitions to `status: 'in-review'`. The action is logged to the audit trail.

---

### § AC-E-028 — sE07: MA 3-day advance notification is auto-sent

**Screen:** sE07 — Content Calendar
**FR:** FR-E-016

**Given** a content card is scheduled for a publishing date
**When** the scheduling is confirmed
**Then** the platform calculates the MA advance notice date (publishing date minus 3 days, configurable). The MA Team Lead and all MA team members receive an email and in-app notification on that date. The calendar entry for cal-005 (5 Nov) shows "MA notified 2 Nov ✓."

---

### § AC-E-029 — sE07: KOL reminder escalation is shown in MA approval

**Screen:** sE06 (referenced from sE07 context)
**FR:** FR-E-013

**Given** a KOL invitation has been sent and 7 days have passed with no response
**When** the Medical Affairs Approval screen is viewed
**Then** the "KOL reminder escalation record" panel shows: "⚠ Escalated to Ideation Lead and Medical Affairs Team Lead on [date] — KOL review stalled" with reminder 1 and reminder 2 timestamps. When the KOL responded within 5 days, the non-escalated variant reads: "no escalation triggered — Prof. James Hartley responded within 5 days."

---

### § AC-E-030 — sE08: Verbatim flagged comment is shown with MA verdict

**Screen:** sE08 — Publishing Monitor & Social Listening
**FR:** FR-E-017

**Given** a social listening alert has a flagged comment
**When** the publishing monitor right panel shows the alert
**Then** the full verbatim text of the flagged comment is shown with: author handle, channel, timestamp, auto-category, auto-audience, and MA notification record. The MA review verdict is shown in full: verdict label, SmPC reference, measurement, threshold, and conclusion. Timestamp of MA review is shown.

---

### § AC-E-031 — sE08: Auto-stop threshold is shown but not triggered

**Screen:** sE08 — Publishing Monitor & Social Listening
**FR:** FR-E-017

**Given** a sentiment alert has a score below the auto-stop threshold
**When** the auto-stop assessment section is displayed
**Then** the section shows: negative sentiment measured, auto-stop threshold, and the verdict "Auto-stop not triggered — sentiment below threshold." Next scheduled content on the same topic is listed with the note that a sentiment check will run before publishing.

---

### § AC-E-032 — sE08: Manual publishing note is visible (DD-E-005)

**Screen:** sE08 — Publishing Monitor & Social Listening
**FR:** FR-E-016, DD-E-005

**Given** the Publishing Monitor screen is displayed
**When** any publishing record is viewed
**Then** the note "Content is published manually by the Creative Team following the platform notification. Direct channel publishing is not enabled in this release." is visible on the screen. No "Publish now" button or direct channel API integration exists.

---

### § AC-E-033 — sE09: Compliance posture note is present

**Screen:** sE09 — Final Output & Publishing Record
**FR:** FR-E-018

**Given** the Final Output & Publishing Record screen is displayed
**When** the sign-off record section is viewed
**Then** the note reads: "All events above are logged to the 21 CFR Part 11 compliant platform audit trail. The KOL and Medical Affairs sign-offs are digital approval stamps, not full Part 11 e-signatures, per the Ideation & Publishing compliance posture."

---

### § AC-E-034 — sE09: Master Library push is recorded with timestamp

**Screen:** sE09 — Final Output & Publishing Record
**FR:** FR-E-018

**Given** the Ideation Lead has pushed approved content cards to the Master Library
**When** the Master Library contribution section is viewed
**Then** the pushed state shows: "Pushed to Master Library ✓ · X cards available for reuse, tagged [TA · Module · Project · published month]." with Ideation Lead name and timestamp. Immutability note: "Published cards and their provenance are immutable. Reuse creates a new card that links back to this record."

---

### § AC-E-035 — sE09: Bridge link to sE10 standards screen

**Screen:** sE09 — Final Output & Publishing Record
**FR:** FR-E-019

**Given** a long-form content card (blog post, article) has been published
**When** the standards section is shown in sE09
**Then** the note states: "[Card name] qualifies as a long-form article and is eligible for DOI registration, Dublin Core tagging and a WCAG 2.1 AA output check." An "Open standards & metadata →" link navigates to sE10 with the card pre-selected.

---

### § AC-E-036 — sE10: DOI registration is irreversible (DD-E-006)

**Screen:** sE10 — Standards, Metadata & DOI
**FR:** FR-E-019, DD-E-006

**Given** the user is about to register a DOI
**When** the registration button is displayed
**Then** a warning is shown: "Registration is irreversible. The DOI is permanent once assigned." The user must explicitly confirm before the `POST /api/ideation-cards/:id/doi/register` call is made. Once registered, the DOI field is read-only and the registration cannot be undone.

---

### § AC-E-037 — sE10: WCAG 2.1 vs 2.2 distinction is explicit

**Screen:** sE10 — Standards, Metadata & DOI
**FR:** FR-E-019 (AC-E-019 supplement)

**Given** the WCAG check panel is displayed
**When** the user reads the panel header
**Then** the header reads "WCAG 2.1 Level AA — Content Output Check" (not WCAG 2.2). The explanatory note states: "This is the content output accessibility standard, WCAG 2.1 level AA. The platform interface itself targets WCAG 2.2 AA; this check applies only to the published PDF and HTML article output."

---

### § AC-E-038 — sE10: Module E owns CrossRef/ORCID shared service

**Screen:** sE10 — Standards, Metadata & DOI
**FR:** FR-E-019, OQ-E-006

**Given** the DOI registration panel is displayed
**When** the user reads the service attribution note
**Then** the note states: "Crossref and ORCID API: this module owns the shared service. Scientific Writing publications use the same service · open question resolved Sept 2026." The note confirms OQ-E-006 is resolved and Module E is the owner.

---

### § AC-E-039 — sE10: Dublin Core metadata is first implementation note

**Screen:** sE10 — Standards, Metadata & DOI
**FR:** FR-E-019

**Given** Dublin Core metadata has been tagged for a content card
**When** the tag completion state is shown
**Then** the note "First implementation of Dublin Core metadata in the platform." is visible, confirming this is the platform's inaugural DC implementation. 15 elements are applied per DCMI terms 2020. Elements are embedded in the PDF and HTML output and stored in the platform, searchable in the Master Library.
...
|---------|------|---------|
| 4.0 | Sept 2026 | AC-D-028 through AC-D-051 added (24 entries — sD01, sD11, sD12, Project Management, T&C). AC-E-024 through AC-E-039 added (16 entries — sE01, sE07, sE08, sE09, sE10). DD-E-006 coverage added. Total ACs: 256 across all five modules. |
