...
- Cross-module consistency check (FR-D-016): complete within 120 seconds for a full NDA/MAA dossier.
...
| `ha_correspondence` | id, submission_id, direction (inbound/outbound), type (loq/response/ack/approval/nack), content_summary, received_at, responded_at, response_doc_id (nullable) | HA communication log |
...
| DD-D-006 | PRD v4.1 §9.1 does not include MHRA gateway — FR-D-021 includes MHRA as Priority 4 pending OQ-D-008 resolution | This PRD includes MHRA based on its explicit presence in PRD v4.1 §5.4 Stage 6, but flags it as subject to OQ-D-008 resolution and a dependency item requiring API procurement. The design sprint for gateway configuration cannot proceed for MHRA until OQ-D-008 is answered. | Sept 2026 |
