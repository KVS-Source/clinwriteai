...
**Modules A, B, C, D & E: Clinical Writing · Scientific Writing · Medical Writing · Regulatory Writing · Ideation & Publishing**
  ...
  theme: {
    extend: {
      ...
      // Module A accent — Clinical Writing
        ...
        // #0D9488 Teal — confirmed from design files sE01–sE10
        // PRD §3.3 listed #005F8E (Steel Blue) — overridden: #005F8E is the cross-module
        // blocking-state colour in C/D and cannot double as a module accent.
        // #E11D48 Rose was a Tailwind stub — overridden: rose is the sE07/sE08 overdue colour.
        teal: {
          600: '#0D9488',
          50:  '#F0FDFA',
          100: '#CCFBF1',
        ...
        // B Scientific Writing:   #7C3AED  (violet — teal was B's stub, now E's confirmed accent)
        // C Medical Writing:      #7C3AED
        // D Regulatory Writing:   #D97706
        // E Ideation & Publishing: #0D9488  ← CONFIRMED
        // AI colours
        'ai-bg':     '#F0F7FF',
        'ai-border': '#93C5FD',
      },
...
| 4.0 | Sept 2026 | Modules D & E appended. §30–§36 cover Module D routes, types, stores, API client, MSW handlers, shared components (ECTDStatusDot, ConsistencyContradictionCard, GatewayACKTimeline, PartElevenConfirm, CTDReadOnlyBanner, DataObjectivityFlag), and five new architectural patterns: canonical JSON indexing, cross-module consistency check (hard gate), continuous eCTD publishing, gateway transmission with Part 11 inline sign, and Regulatory Intelligence as a cross-module shared service. §37–§43 cover Module E routes, types, stores, API client, MSW handlers, shared components (ProvenanceChip, ClaimCurrencyBadge, ChannelAdaptationCard, SourceGateBlock, AtomisationSpinner), and four new architectural patterns: source content gating, AI multi-format atomisation (one call per channel), KOL guest route (public, outside AppShell), and DOI/ORCID/Dublin Core shared service owned by Module E. ESLint boundary extended in §36 to cover all five modules. |
...
### Appendix to Technical Architecture v4.0

---

## 30. Module D Overview

Module D — Regulatory Writing — adds twelve screens to the Aurora prototype (sD01–sD12). It shares the existing platform shell and all Module A/B/C UI primitives. Module D introduces five architectural patterns not present in Modules A–C:

1. **Canonical JSON indexing layer** — structured, version-controlled data index of all source documents (CSRs, IBs, SAPs, TLFs) enabling AI authoring without re-reading full PDFs (FR-D-001)
2. **Cross-module consistency check** — automated contradiction detection between CTD Module 2.5/2.7 text values and Module 5 TFL data points; hard gate before Stage 4 (FR-D-016, DD-D-002)
3. **Continuous eCTD publishing** — documents compile into the eCTD package automatically as they are locked; Stage 5 is validation, not assembly (FR-D-019)
4. **Regulatory gateway transmission with Part 11 inline sign** — FDA ESG / EMA CESP / CDSCO / MHRA; every transmission requires inline credential confirmation before the API call fires (FR-D-021)
5. **Regulatory Intelligence as a cross-module service** — live framework change monitoring owned by Module D, alerts surfaced to all five modules (FR-D-024, DD-D-005)

Module D is scoped to `modules/regulatory-writing/` and must not import from any other module folder. ESLint boundary rule is extended in §36.
...
```typescript
// router/index.tsx — regulatory-writing subtree
{
...
## 32. Module D TypeScript Types

Appended to `packages/types/src/domain.ts`.

```typescript
// ─── Module D domain types ────────────────────────────────────────────────────

export type RegSubmissionType =
  | 'ind' | 'nda-maa' | 'psur-pbrer' | 'rmp-rems'
  | 'ha-response' | 'cer' | 'orphan-drug'
...
export interface ECTDGranularityNode {
  id: string
  submissionId: string
  moduleSection: string           // e.g. '2.5', '3.2.S'
  sectionTitle: string
  status: CTDModuleStatus
  documentId?: string
  aiFootprintPct?: number
  lastUpdated: string
  isReadOnly: boolean             // true for Module 5 nodes (DD-D-001)
  isSystemGenerated: boolean      // true for 2.1, 2.2, cover letters
}
...
export interface ConsistencyCheckResult {
  ...
  passed: boolean                 // true when no major contradictions unresolved
}
...
export interface CMCReadinessReport {
  ...
  missingItems: Array<{ section: string; item: string; required: boolean }>
  generatedAt: string
  acknowledgedBy?: string
  acknowledgedAt?: string
  riskNote?: string
...
export interface RedactionRecord {
  ...
  submissionId: string
  documentId: string
  ppdItems: Array<{ id: string; location: string; text: string; confirmed: boolean; confirmedBy?: string }>
  cciItems: Array<{ id: string; location: string; text: string; confirmed: boolean; confirmedBy?: string }>
  irreversibleAfterStage5: true
}
...
export interface GatewaySubmissionRecord {
  ...
  partEleven: { credentialHash: string; meaning: string; timestamp: string }
}
...
export interface ODDAssessment {
  ...
  prevalenceMeetsThreshold: boolean
  benefitDraft: string
  clinicalLeadSignedAt?: string
}

// ─── Module D lookup constants ─────────────────────────────────────────────────

export const GATEWAY_PRIORITY: Record<GatewayTarget, { label: string; priority: number; apiReady: boolean }> = {
  'fda-esg':  { label: 'FDA ESG',   priority: 1, apiReady: true  },
  'ema-cesp': { label: 'EMA CESP',  priority: 2, apiReady: true  },
  'cdsco':    { label: 'CDSCO',     priority: 3, apiReady: true  },
  'mhra':     { label: 'MHRA',      priority: 4, apiReady: false }, // prototype UI only — OQ-D-008 resolved
}

export const CTD_STATUS_META: Record<CTDModuleStatus, { symbol: string; colour: string; label: string }> = {
  'not-started':     { symbol: '○', colour: '#94A3B8', label: 'Not started'       },
  'in-authoring':    { symbol: '●', colour: '#B0200D', label: 'In authoring'       },
  'in-review':       { symbol: '▲', colour: '#B45309', label: 'In review'          },
  'signed':          { symbol: '✓', colour: '#15803D', label: 'Signed off'         },
  'auto-generated':  { symbol: '◉', colour: '#005F8E', label: 'System-generated'   },
  'read-only':       { symbol: '🔒', colour: '#475569', label: 'Read-only · Module A' },
}
```

---

## 33. Module D State Management (Zustand)

Five new stores in `src/modules/regulatory-writing/store/`:

```typescript
// regulatorySubmissionStore.ts — submission list + active submission
interface RegulatorySubmissionStore {
  submissions: RegulatorySubmission[]
  activeSubmission: RegulatorySubmission | null
  activePanel: 'consistency' | 'cross-ref' | 'audit' | 'ai-suggest' | null
  fetchSubmissions: (projectId: string) => Promise<void>
  setActiveSubmission: (s: RegulatorySubmission) => void
}

// ectdStore.ts — live eCTD map
interface ECTDStore {
  nodes: ECTDGranularityNode[]
  selectedNodeId: string | null
  publishingLog: string[]
  isCompiling: boolean
  fetchNodes: (submissionId: string) => Promise<void>
  selectNode: (id: string | null) => void
}

// superReviewStore.ts — Stage 4 six-role review
interface SuperReviewStore {
  reviewers: Array<{ role: string; userId: string; signedAt?: string }>
  comments: Array<{ id: string; role: string; text: string; resolvedAt?: string }>
  consistencyResult: ConsistencyCheckResult | null
  activeTab: 'consistency' | 'safety-report' | 'comments'
  runConsistencyCheck: (submissionId: string) => Promise<void>
  resolveContradiction: (id: string, note: string) => Promise<void>
  signOff: (submissionId: string, role: string) => Promise<void>
}

// gatewayStore.ts — transmission + ACK tracking
interface GatewayStore {
  records: GatewaySubmissionRecord[]
  confirmingTransmission: boolean   // gates the API call — must be true for inline Part 11 confirm
  setConfirmingTransmission: (v: boolean) => void
  transmit: (submissionId: string, gateway: GatewayTarget, body: object) => Promise<void>
}

// regulatoryIntelligenceStore.ts — cross-module shared service
...
## 34. Module D API Client

File: `src/modules/regulatory-writing/api/regulatoryWriting.ts`. 32 endpoints (API Contracts v4.0 §44–§56).

Key client methods:
```typescript
export const regulatoryWritingApi = {
  // Submissions
  list:                 (projectId: string) => api.get<RegulatorySubmission[]>(`/projects/${projectId}/regulatory-submissions`),
  get:                  (id: string) => api.get<RegulatorySubmission>(`/regulatory-submissions/${id}`),
  create:               (projectId: string, body: object) => api.post<RegulatorySubmission>(`/projects/${projectId}/regulatory-submissions`, body),
  advanceStage:         (id: string) => api.post<RegulatorySubmission>(`/regulatory-submissions/${id}/advance-stage`, {}),
  // Canonical JSON
  indexCanonicalJSON:   (id: string) => api.post<{ dataPoints: number }>(`/regulatory-submissions/${id}/canonical-json/index`, {}),
  // eCTD map
  getECTDMap:           (id: string) => api.get<ECTDGranularityNode[]>(`/regulatory-submissions/${id}/ectd-map`),
  // CMC readiness
  getCMCReadiness:      (id: string) => api.get<CMCReadinessReport>(`/regulatory-submissions/${id}/cmc-readiness`),
  acknowledgeCMC:       (id: string, riskNote: string) => api.post(`/regulatory-submissions/${id}/cmc-readiness/acknowledge`, { riskNote }),
  // Consistency check (120s simulated latency)
  runConsistencyCheck:  (id: string) => api.post<ConsistencyCheckResult>(`/regulatory-submissions/${id}/consistency-check/run`, {}),
  resolveContradiction: (id: string, contradictionId: string, note: string) => api.patch(`/regulatory-submissions/${id}/consistency-check/contradictions/${contradictionId}/resolve`, { note }),
  // PPD/CCI redaction (irreversible after Stage 5 — DD-D-003)
  getRedaction:         (id: string) => api.get<RedactionRecord>(`/regulatory-submissions/${id}/redaction`),
  confirmRedaction:     (id: string, itemId: string, type: RedactionType) => api.patch(`/regulatory-submissions/${id}/redaction/${type}/${itemId}/confirm`, {}),
  // eCTD validation (300s simulated)
  runECTDValidation:    (id: string) => api.post<object>(`/regulatory-submissions/${id}/ectd-validation/run`, {}),
  // Gateway (Part 11 required — DD confirmingTransmission gate)
  transmit:             (id: string, gateway: GatewayTarget, body: object) => api.post<GatewaySubmissionRecord>(`/regulatory-submissions/${id}/gateway/transmit`, { gateway, ...body }),
  // HA response
  uploadLoQ:            (id: string, file: FormData) => api.post(`/regulatory-submissions/${id}/ha-correspondence/loq`, file),
  generateHAResponse:   (id: string, questionId: string) => api.post<object>(`/regulatory-submissions/${id}/ha-response/${questionId}/generate`, {}),
  // Regulatory Intelligence (cross-module service)
  getAlerts:            () => api.get<RegAlert[]>('/regulatory-alerts'),
  acknowledgeAlert:     (alertId: string) => api.patch(`/regulatory-alerts/${alertId}/acknowledge`, {}),
  // Master Library push (after ACK2)
  pushToLibrary:        (id: string, body: object) => api.post(`/regulatory-submissions/${id}/library/push`, body),
}
```

---

## 35. Module D MSW Handlers

File: `src/mocks/handlers/regulatoryWriting.ts`. Add to `src/mocks/browser.ts` after `...medContentHandlers`.

```typescript
// src/mocks/browser.ts — updated
...
**JSON fixture files for `/src/data/`:**

| File | Records | Key demo data |
|------|---------|--------------|
| `regulatorySubmissions.json` | 3 | VELORA-301 NDA (Stage 3 authoring), VELORA-301 PSUR (Stage 4 Super Review), AURELIA-101 IND (Stage 6 Submitted ACK2) |
| `ectdGranularityMap.json` | 62 nodes | Full CTD tree — 47 compiled ✓, 8 in-authoring, 4 pending, 3 auto-generated |
| `cmcReadinessReport.json` | 1 | 84% complete — stability data gap in 3.2.A acknowledged |
| `consistencyCheckResult.json` | 1 | 2 contradictions: HR 0.61 vs 0.63 (Major), median PFS 9.7 vs 9.4 months (Minor) |
| `regulatoryAlerts.json` | 2 | ICH E2C(R2) draft revision, FDA 21 CFR Part 314 update |
| `gatewaySubmissions.json` | 2 | FDA ESG ACK2 ✓, EMA CESP pending |

---

## 36. Module D ESLint Boundary Update
...
**DD-D-004 note (HA response grounding):** The `generateHAResponse` API client method grounds every AI-drafted HA response in the canonical JSON layer from FR-D-001 — not re-derived from the PDF dossier. This is enforced in the API client by always including the `canonicalJsonLayerId` in the request body. See API Contracts §52.

**Module D shared components added to `src/components/ui/`:**

| Component | Props | Purpose |
|-----------|-------|---------|
| `ECTDStatusDot.tsx` | `{ status: CTDModuleStatus }` | Symbol + colour dot for eCTD tree nodes |
| `ConsistencyContradictionCard.tsx` | `{ contradiction: ConsistencyContradiction }` | Source/target value diff card with resolve input |
| `GatewayACKTimeline.tsx` | `{ record: GatewaySubmissionRecord }` | ACK1/2/3 milestone timeline with elapsed times |
| `PartElevenConfirm.tsx` | `{ onConfirm: () => void; meaning: string }` | Inline Part 11 confirmation — gates all gateway calls |
| `CTDReadOnlyBanner.tsx` | none | Full-width Module 5 read-only banner (DD-D-001) |
| `DataObjectivityFlag.tsx` | `{ text: string; suggestion: string }` | Purple dashed underline with tooltip for Module 2 editor |
...
## 37. Module E Overview

Module E — Ideation & Publishing — adds ten screens to the Aurora prototype (sE01–sE10). It is the only module without a regulated document editor (DD-E-001). Module E introduces four architectural patterns not present in Modules A–D:

1. **Source content gating** — before any artefact enters the ideation pipeline, the system verifies it has "Signed / Final Output / Submitted" status in its originating module; external uploads require explicit Ideation Lead confirmation (FR-E-003, DD-E-002)
2. **AI multi-format atomisation** — one structured AI call per channel format; each adaptation is editable by the Ideation Lead; source document is always read-only (FR-E-007, DD-E-001, DD-E-004)
3. **KOL guest route** — `/kol-review/:token` is a public route outside the authenticated AppShell; token is a one-time JWT valid 7 days; KOLs have no Aurora account (FR-E-010)
4. **Master Library 90-day currency rule** — Master Library cards pulled within 90 days bypass the source currency check automatically (FR-E-002 v0.3 fix); cards older than 90 days receive a lightweight check before being added to the ideation project
5. **DOI/ORCID/Dublin Core shared service** — Module E owns the CrossRef and ORCID APIs; Module B is a consumer of the same service; Dublin Core metadata is embedded in published PDF/HTML outputs (FR-E-019, DD-E-006, OQ-E-006 resolved)

Module E is scoped to `modules/ideation-publishing/` and must not import from any other module folder. ESLint boundary rule is in §36.
...
```typescript
// router/index.tsx — ideation-publishing subtree
{
...
// KOL guest route — PUBLIC, outside AppShell
{ path: 'kol-review/:token',              element: <KOLReviewInterface /> },         // sE05
```

---

## 39. Module E TypeScript Types

Appended to `packages/types/src/domain.ts`.

```typescript
// ─── Module E domain types ────────────────────────────────────────────────────
...
export type ChannelFormat =
  ...
  | 'hcp' | 'medical-affairs' | 'instagram' | 'facebook'
...
export interface IdeationProject {
  id: string
  projectId: string
  sourceType: 'master-library' | 'upload' | 'external'
  taTag: string
...
export interface IdeationContentCard {
  ...
  // Denormalised for query performance — derivation rule:
...
export interface KOLContact {
  ...
  reviewLinkToken: string          // one-time JWT, expires 7 days
...
export interface DublinCoreMetadata {
  ...
  dcTitle: string
  dcCreator: string
  dcSubject: string               // TA tag
  dcDescription: string
  dcDate: string
  dcType: string
  dcFormat: string
  dcIdentifier: string            // DOI if registered
  dcRights: string
  taggedAt: string
}
...
## 40. Module E State Management (Zustand)

Four new stores in `src/modules/ideation-publishing/store/`:

```typescript
// ideationStore.ts
interface IdeationStore {
  projects: IdeationProject[]
  activeProject: IdeationProject | null
  activeArtefact: IdeationArtefact | null
  cards: IdeationContentCard[]
  sourceGateResult: { passed: boolean; reason?: string } | null
  claimCurrencyResult: object | null
  fetchProjects: (projectId: string) => Promise<void>
  runSourceGate: (artefactId: string) => Promise<void>
  runClaimCurrency: (artefactId: string) => Promise<void>
  runSourceCurrency: (artefactId: string) => Promise<void>
}

// atomisationStore.ts
interface AtomisationStore {
  adaptations: Record<ChannelFormat, AtomisedContent | null>
  generatingChannels: ChannelFormat[]     // per-channel loading state
  complianceResults: object | null
  atomise: (cardId: string, channel: ChannelFormat) => Promise<void>   // one call per channel (DD-E-004)
  runComplianceScreen: (cardId: string) => Promise<void>
}
...
interface CalendarStore {
  ...
  publishingRecords: object[]
  socialAlerts: SocialListeningAlert[]
  setView: (v: 'month' | 'week') => void
  scheduleCard: (cardId: string, date: string, channel: ChannelFormat) => Promise<void>
  markPublished: (entryId: string, utmParams: string, seoMetadata: object) => Promise<void>
}

// standardsStore.ts
interface StandardsStore {
  doiRecord: DOIRecord | null
  dublinCoreRecord: DublinCoreMetadata | null
  orcidVerifications: Array<{ name: string; orcid: string; verified: boolean }>
  wcagResult: object | null
  registerDOI: (cardId: string, body: object) => Promise<void>
  verifyORCID: (orcid: string) => Promise<boolean>
  tagDublinCore: (cardId: string, body: object) => Promise<void>
  runWCAG: (cardId: string, format: string) => Promise<void>
}
```

---

## 41. Module E API Client
...
export const ideationPublishingApi = {
  // Projects + artefacts
  list:                 (projectId: string) => api.get<IdeationProject[]>(`/projects/${projectId}/ideation`),
  create:               (projectId: string, body: object) => api.post<IdeationProject>(`/projects/${projectId}/ideation`, body),
  // Source checks (sequential: gate → currency → claim)
  runSourceGate:        (artefactId: string) => api.post(`/ideation-artefacts/${artefactId}/source-gate`, {}),
  runSourceCurrency:    (artefactId: string) => api.post(`/ideation-artefacts/${artefactId}/source-currency`, {}),
  runClaimCurrency:     (artefactId: string) => api.post(`/ideation-artefacts/${artefactId}/claim-currency`, {}),
  acknowledgeFlag:      (artefactId: string, flagId: string, note: string) => api.patch(`/ideation-artefacts/${artefactId}/flags/${flagId}/acknowledge`, { note }),
  // Content cards
  getCards:             (projectId: string) => api.get<IdeationContentCard[]>(`/ideation/${projectId}/cards`),
  createCard:           (projectId: string, body: object) => api.post<IdeationContentCard>(`/ideation/${projectId}/cards`, body),
  // Atomisation — one call per channel (DD-E-004)
  atomise:              (cardId: string, channel: ChannelFormat) => api.post<AtomisedContent>(`/ideation-cards/${cardId}/atomise`, { channel }),
  // Compliance screen
  runComplianceScreen:  (cardId: string) => api.post(`/ideation-cards/${cardId}/compliance-screen`, {}),
  // KOL
  inviteKOL:            (projectId: string) => api.post(`/ideation/${projectId}/kol/invite`, {}),
  submitKOLReview:      (token: string, decisions: object[]) => api.post(`/kol-review/${token}/submit`, { decisions }),
  // MA approval
  maApprove:            (projectId: string, cardIds: string[]) => api.post(`/ideation/${projectId}/ma/approve`, { cardIds }),
  // Calendar
  schedule:             (body: object) => api.post('/ideation-calendar/schedule', body),
  markPublished:        (entryId: string, body: object) => api.post(`/ideation-calendar/${entryId}/mark-published`, body),
  // Standards (DOI/ORCID/Dublin Core/WCAG 2.1)
  registerDOI:          (cardId: string, body: object) => api.post<DOIRecord>(`/ideation-cards/${cardId}/doi/register`, body),
  verifyORCID:          (orcid: string) => api.get(`/orcid/verify/${orcid}`),
  tagDublinCore:        (cardId: string, body: object) => api.post<DublinCoreMetadata>(`/ideation-cards/${cardId}/dublin-core/tag`, body),
  runWCAG:              (cardId: string, format: string) => api.post(`/ideation-cards/${cardId}/wcag/run`, { format }),
  // Master Library push
  pushToLibrary:        (projectId: string, body: object) => api.post(`/ideation/${projectId}/library/push`, body),
}
```

---

## 42. Module E MSW Handlers

File: `src/mocks/handlers/ideationPublishing.ts`. Add to `src/mocks/browser.ts` after `...regulatoryWritingHandlers`.

```typescript
// src/mocks/browser.ts — final complete registration
export const worker = setupWorker(
  ...documentHandlers, ...projectHandlers, ...aiHandlers,
  ...publicationHandlers, ...medContentHandlers,
...
| File | Records | Key demo data |
|------|---------|--------------|
| `ideationProjects.json` | 3 | VELORA-301 ideation (Stage 4 Approved), AURELIA-101 ideation (Stage 2 Under Review), VELORA-302 ideation (Stage 1 Uploaded) |
| `ideationArtefacts.json` | 3 | KOL Session Summary (Module C, approved), Phase I Brief (External, confirmed), VELORA-302 draft (BLOCKED — in-authoring, source gate failed) |
| `ideationContentCards.json` | 3 | C-001 Efficacy (approved, provenance chain), C-002 Safety Profile (approved), C-003 Subgroup (approved, claim currency warning acknowledged) |
| `atomisedContent.json` | 8 | LinkedIn (with compliance fix), Blog Post, HCP Summary, Email, X/Twitter — for all 3 cards where applicable |
| `claimCurrencyCheck.json` | 1 | 12 current, 1 potentially superseded, 1 conflicting (acknowledged) |
| `ideationCalendar.json` | 5 | LinkedIn 22 Oct ✓, Blog Post 25 Oct (late), HCP 28 Oct ✓, Overdue 18 Oct, Scheduled 5 Nov |
| `socialListeningAlerts.json` | 1 | Blog Post negative sentiment (resolved by MA Lead) |

---

## 43. Module E Shared Components

Added to `src/components/ui/`:

| Component | Props | Purpose |
|-----------|-------|---------|
| `ProvenanceChip.tsx` | `{ chain: string[] }` | Inline source chain: "Source: Doc §3.2 → CSR v1.0 → Module A" |
| `ClaimCurrencyBadge.tsx` | `{ status: ClaimCurrencyStatus }` | Current (green) / Potentially Superseded (amber) / Conflicting (steel blue) |
| `ChannelAdaptationCard.tsx` | `{ adaptation: AtomisedContent; onEdit: () => void }` | Editable channel-specific content card with AI footprint chip |
| `SourceGateBlock.tsx` | `{ reason: string; originModule: string }` | Full-width block state for failed source content gate |
| `AtomisationSpinner.tsx` | `{ channels: ChannelFormat[]; completing: ChannelFormat[] }` | Per-channel spinners (DD-E-004 pattern) |


---

## 44. Project Management Layer (PRD §3A)

A Platform-level Project entity sits above all five modules. Every action on the platform occurs within a named project. Projects are created by users with `project-lead` role or above.

### Project Entity Fields

| Field | Type | Required | Notes |
|-------|------|----------|-------|
| `projectId` | `string` (UUID) | ✓ | Auto-assigned on creation |
| `projectName` | `string` | ✓ | Unique within client account |
| `startDate` | `ISO8601 date` | ✓ | Cannot be a past date |
| `customerName` | `string` | ✓ | Linked to Admin-configured client master |
| `modulesSelected` | `ModuleKey[]` | ✓ | `['A','B','C','D','E']` — checkbox multi-select |
| `therapeuticArea` | `string[]` | ✓ | Mandatory; multi-select from managed taxonomy |
| `endDate` | `ISO8601 date` | — | Optional |
| `projectDescription` | `string` | — | Optional free text |
| `linkedStudyRef` | `string` | — | Optional |
| `countriesOfSubmission` | `string[]` | — | Optional multi-select |
| `status` | `ProjectStatus` | ✓ | See §44.1 |
| `projectLead` | `UserId` | ✓ | Set at creation |
| `createdAt` | `ISO8601` | ✓ | Auto |
| `closedAt` | `ISO8601 \| null` | — | Set when status → Closed |

### 44.1 Project Statuses

```typescript
type ProjectStatus = 'Initiated' | 'Ongoing' | 'On Hold' | 'Re-open' | 'Closed'
```

| Status | Trigger | Platform Behaviour |
|--------|---------|-------------------|
| `Initiated` | Project created | Modules unlocked per selection; no documents yet |
| `Ongoing` | First document created | All modules active; costs and time tracking active |
| `On Hold` | Manually set by Project Lead or Admin | All active work paused; reason recorded |
| `Re-open` | Manually triggered to reactivate | Returns to `Ongoing` state |
| `Closed` | Project Lead or Admin marks closed | **Hard system rule: read-only for all users including Super Admin. Enforced at API layer. Cannot be overridden.** |

### 44.2 All Projects Tab

A dedicated **All Projects** tab is visible on the main dashboard to all authenticated users. Columns: Project Name · Project ID · Customer Name · Modules Selected (coloured dots) · Therapeutic Area · Start Date · Status · Project Lead. Filters: Status, TA, Module, Customer, Date Range. Clicking a row navigates to the Project Dashboard.

### 44.3 Project Dashboard

The Project Dashboard shows:
- All documents across all selected modules in a consolidated view
- **Cost summary:** total services consumed, total units, total spend to date, total market-value savings
- **Time summary:** total days elapsed since Start Date, average stage completion time per module
- **Performance:** composite health score (weighted average of module health scores), review pass rate, CRM completion rate
- Project-level PDF/CSV report: generated by Project Lead role or above

Therapeutic area tag(s) are displayed in the Project Dashboard header and mandatory on every document within the project.

---

## 45. Module E Roles

All seven Module E roles from PRD §6.5 and §19.4:

| Role | User ID convention | Primary Responsibility |
|------|-------------------|----------------------|
| Ideation Lead | `user-il` | Uploads artefacts; tags content; manages ideation review cycle; records voice notes |
| Voice Note Contributor | `user-vnc` | Records voice notes against ideation content (up to **3 per project per ideation cycle**) |
| KOL (Key Opinion Leader) | Guest — no Aurora account | Reviews content cards; approves/rejects; accessed via secure one-time link |
| Medical Affairs Team Lead | `user-ma` | Reviews and approves final content before calendar publishing; receives 3-day advance notifications |
| Medical Affairs Team Member | `user-ma-member` | Receives publishing due date notifications; reviews content |
| Creative / Marketing Team Member | `user-cr` | Executes actual publishing of approved content on scheduled dates |
| Content Calendar Manager | `user-ccm` | Sets publishing dates; assigns content to calendar slots; manages publishing schedule |

**3-user voice note limit:** The platform enforces a maximum of 3 Voice Note Contributors per project per ideation cycle. A 4th contributor receives a 409 response from the API. This limit is enforced at the API layer and cannot be overridden.

---

## 46. Module E Notification Events

Five notification types from PRD §19.5:

| Event | Trigger | Recipients | Channel |
|-------|---------|------------|---------|
| Stage advance | Content card moves Uploaded→Under Review→Reviewed→Approved | All role holders for that stage | Email + in-app |
| KOL review invitation | Content enters Under Review | KOL (external email) | Email — secure one-time link |
| MA due date advance notice | Configurable days before scheduled publish date (default: 3 days) | All MA team members (name + email + mobile) | Email + in-app + SMS (if mobile captured) |
| Publishing overdue alert | Content not published within 24h of scheduled date | MA Team Lead + Content Calendar Manager | Email + in-app |
| Mobile/SMS | Time-sensitive: publishing due, KOL review request | MA team + KOLs with mobile captured | SMS (via configurable SMS gateway in Admin) |

KOL invitation email includes: project name, document title, TA tag, content section(s) for review, KOL name, and a secure one-time link. Link expires in 7 days.

---

## 47. Content Calendar Localisation (sE07)

Each approved content card can be localised for specific territories. The Localisation panel in sE07 shows:

```typescript
interface LocaleRecord {
  id: string
  ideationContentCardId: string
  localeCode: string          // e.g. 'fr-FR', 'de-DE'
  country: string             // e.g. 'France', 'Germany'
  status: 'pending' | 'in-review' | 'approved' | 'published'
  affiliateContactEmail: string
  localPDFPath: string | null
  detail: string              // notes on local regulatory status
}
```

Local MA review links are sent by email to affiliate contacts with the localised PDF and instructions in the local language. Every localised version links back to the parent approved card record, preserving the full provenance chain.

**MSW handler:** `GET /api/ideation-cards/:id/locales` → `LocaleRecord[]` | `POST /api/ideation-cards/:id/locales` → `LocaleRecord`

---

## 48. FR-E-013 — KOL Escalation Notification

**FR-E-013:** When a KOL invitation has been sent and no review response is received within the configured period, the platform automatically escalates.

Escalation cadence (configurable; defaults in PRD §19.5):
- Reminder 1: 3 days after invitation
- Reminder 2: 5 days after invitation
- Escalation to Ideation Lead + MA Team Lead: 7 days with no response

The KOL escalation record is written to the audit trail and surfaced in sE06 (Medical Affairs Approval) in the "KOL reminder escalation record" panel. Both the escalated and non-escalated states are rendered (the design shows both variants).

---

## 49. FR-D-002 — eCTD Version Selection

**FR-D-002:** At submission setup (sD02), the Admin-configured eCTD version is pre-selected but overridable by the Regulatory Affairs Lead for the specific submission. Available options:
- `eCTD v3.2.2` (current standard — FDA, EMA, PMDA)
- `eCTD v4.0` (emerging — PMDA mandatory; FDA/EMA transition)

The selected version drives eCTD Granularity Map validation rules in sD03 and publishing package assembly in sD07. Version is locked once the submission enters Stage 3.

---

## 50. FR-D-023 — Cross-Module Safety Signal Trigger

**FR-D-023:** When a safety signal is flagged in Module D (Regulatory Intelligence, sD11), the platform automatically creates a label update task in SmPC §4.4 (Special Warnings) and USPI (Warnings and Precautions). The trigger is recorded in the cross-functional triggers panel in sD11 with state "● In progress" and assigned to the PV Lead role.

This is the FR-D-025 key demo moment in sD11: "Safety signal flagged — potential hepatotoxicity signal. Label update task auto-created: SmPC §4.4 Special Warnings."

---

## 51. FR-D-025 — Regulatory Framework Alert Push (Cross-Module)

**FR-D-025:** When a regulatory framework alert is triggered in sD11, the alert is automatically pushed to all other modules where the framework applies to their content. The alert card shows which modules received the push. This is the cross-module ownership rule from DD-D-005.

Example: ICH E2C(R2) alert → pushed to Regulatory Writing (Module D) + Medical Writing (Module C).

---

## 52. DD-E-003 — Conflicting Claim Blocks Tagging

**DD-E-003:** When a claim in an uploaded artefact is flagged as `status: 'conflicting'` by the claim currency check (sE02), the tagging action for that specific passage is **absent** (not disabled) from the Content Card Tagging screen (sE03). The Ideation Lead cannot proceed until the claim is resolved at source. This is a hard block at the passage level, not at the project level.

---

## 53. DD-E-005 — Human-Executed Publishing

**DD-E-005:** Publishing in the Ideation & Publishing module is human-executed. The platform notifies the Creative/Marketing Team Member on the scheduled date; that person executes the actual post on the external channel. The platform records the publish event via the "Mark as published" action. **The platform does not call any social platform API directly.** This applies to all channels: LinkedIn, X/Twitter, Instagram, Facebook, blog platforms.

---

## 54. Regulatory Framework Registry (PRD §11A.4)

Module D maintains awareness of the following regulatory frameworks. These drive the framework monitor table in sD11 (Regulatory Intelligence):

| Framework | Issuer | Version | Module D Scope |
|-----------|--------|---------|---------------|
| 21 CFR Part 11 | FDA | Current | Mandatory — digital signatures and audit trail |
| 21 CFR Part 312 | FDA | Current | IND submission requirements |
| 21 CFR Part 314 | FDA | Current | NDA submission requirements |
| ICH M4E(R2) | ICH | Jun 2022 | Mandatory — CTD structure and eCTD format |
| ICH M2 | ICH | v3.2.2 / v4.0 | eCTD technical specification |
| ICH Q8, Q9, Q10, Q11 | ICH | Current | CMC data requirements (Module 3) |
| ICH S1–S9 | ICH | Current | Nonclinical study design (Module 4) |
| ICH E2C(R2) | ICH | Nov 2012 | PBRER/PSUR benefit-risk framework |
| EU Reg 726/2004 | EMA | 2004 (amended) | Centralised MAA submission |
| EU GMP Annex 11 | EMA | Feb 2011 | Computerised systems; electronic records |
| GSPR (MDR/IVDR) | EU | MDR 2017/745 | Clinical Evaluation Reports (CERs) |
| ICH E2F (DSUR) | ICH | Jul 2011 | Development Safety Update Reports |
| ICH E2A | ICH | Oct 1994 | Clinical safety data management |
| EMA GVP Module V | EMA | 2014 | Risk management plans |
| GDPR (EU) 2016/679 | EU | 2018 | Data protection — all submission content |

The framework registry is read-only in the platform UI. The Super Admin can update framework entries via Admin settings.

---

## 55. Non-Functional Requirements (Additions from PRD §13)

Supplementing the existing NFRs:

- **GxP Compliance:** All document authoring and approval workflows comply with GxP computerised system validation requirements (GAMP 5). Audit trails are GxP-compliant — every state change records who, what, when, and why.
- **ISO 27001:** Information security management system. All data at rest and in transit is encrypted. Annual penetration testing conducted against OWASP Top 10. Results reviewed by CISO.
- **SOC 2 Type II:** Platform undergoes annual SOC 2 Type II audit. Audit reports available to enterprise clients under NDA.
- **Penetration Testing:** Annual third-party penetration test. Critical/high findings remediated within 30/90 days respectively.
- **Uptime SLA:** 99.9% uptime target (excluding scheduled maintenance). Downtime measured per calendar month. Scheduled maintenance windows communicated 72h in advance.

---

## 56. Terms & Conditions Screen (PRD §20)

A Terms & Conditions acceptance screen is presented at first sign-up and on material T&C changes. The screen includes:
- Full T&C text with section navigation
- Regulatory disclaimer: "This platform is a productivity and authoring tool. It is not a substitute for regulatory advice. All content generated must be reviewed by qualified regulatory professionals before submission to any health authority." (PRD §20.2)
- Checkbox: "I have read and agree to the Terms & Conditions and Regulatory Disclaimer" — mandatory
- Submit button disabled until checkbox is checked
- Acceptance is written to the user audit trail with timestamp

The regulatory disclaimer also appears in the platform footer on all screens.

---

## 57. Master Library — Complete Specification (PRD §7.6 UPDATED)

Additions to the Master Library spec:

- **Partial reuse:** Authors can pull specific tagged sections (not just whole documents) from the library into a new document. The section retains its full provenance chain when reused.
- **Best Practices library:** A sub-collection maintained by the Super Admin, refreshed quarterly. Items follow the naming convention: `[Module]-[Category]-[Version]-[Date]`. Accessible to all users with appropriate permissions but not editable by non-Super Admin users.
- **Platform API access:** Third parties can access the Master Library via the Platform API (PRD §9.2). Read-only access to approved public items. Authentication via API key issued by Admin.
- **Tagged sections:** Only sections that have been explicitly tagged and pushed appear in the library. Every item tagged with: Module, Project Name, Therapeutic Area, Document Type, Version, optional free-text tags.
- **Archive on close:** When a project is closed, its library contributions remain available in read-only form, tagged with project name and closure date.


---

## 58. Platform & Admin Routes (sPM04–sPM18 + sB10)

Add the following routes to `src/router/index.tsx` inside the `AuthGuard + AppShell` tree:

```typescript
// ── Platform routes (Admin role and above) ────────────────────────────────
{ path: 'library',                    element: <AdminGuard><MasterLibrary /></AdminGuard> },           // sPM08
{ path: 'library/best-practices',     element: <SuperAdminGuard><BestPracticesLibrary /></SuperAdminGuard> }, // sPM09
{ path: 'notifications',              element: <NotificationCentre /> },                               // sPM14
{ path: 'reports',                    element: <AdminGuard><ReportsAnalytics /></AdminGuard> },        // sPM16
{ path: 'services',                   element: <AdminGuard><ServicesDashboard /></AdminGuard> },       // sPM11
{ path: 'onboarding/:module',         element: <OnboardingWizard /> },                               // sPM10

// ── Admin routes (Admin role guard) ──────────────────────────────────────
{ path: 'admin', children: [
    { index: true,                    element: <AdminGuard><AdminPanel /></AdminGuard> },              // sPM04
    { path: 'users',                  element: <AdminGuard><UserManagement /></AdminGuard> },          // sPM06
    { path: 'raci',                   element: <AdminGuard><RACIMatrix /></AdminGuard> },              // sPM07
    { path: 'taxonomy',               element: <AdminGuard><TATagConfiguration /></AdminGuard> },      // sPM17
    { path: 'subscription',           element: <AdminGuard><SubscriptionPayment /></AdminGuard> },     // sPM13
    { path: 'audit',                  element: <AdminGuard><AuditTrailViewer /></AdminGuard> },        // sPM15
    { path: 'frameworks',             element: <AdminGuard><RegulatoryFrameworkAdmin /></AdminGuard> }, // sPM18 read-only for Admin
]},

// ── Super Admin routes (Super Admin role guard) ───────────────────────────
{ path: 'super-admin', children: [
    { index: true,                    element: <SuperAdminGuard><SuperAdminPanel /></SuperAdminGuard> },       // sPM05
    { path: 'rate-card',              element: <SuperAdminGuard><RateCardAdmin /></SuperAdminGuard> },         // sPM12
    { path: 'frameworks',             element: <SuperAdminGuard><RegulatoryFrameworkAdmin /></SuperAdminGuard> }, // sPM18 edit for SuperAdmin
]},

// ── Module B — Slide Deck Generator (add inside scientific-writing routes) ─
// Inside: path 'projects/:projectId' > 'scientific-writing' > 'publications/:publicationId' > children:
{ path: 'slides',                     element: <SlidedeckGenerator /> },                               // sB10
```

**Role guards** — add to `src/platform/components/guards/`:
```typescript
// AdminGuard.tsx — checks currentUser.role is 'admin' or 'super-admin'
// SuperAdminGuard.tsx — checks currentUser.role is 'super-admin' only
// Both redirect to /projects on failure
```
